...
<html>
	...
	<body>
		...
		<div id="app">
			<div class="zyc_content">
				...
				<ul class="zyc_clear zyc_handle">
					<li><button id="zyc_add" class="layui-btn layui-btn-normal"><i class="iconfont iconxinzeng"></i>添加角色</button></li>
				</ul>
				<!-- 页面内容：头部——按钮组 结束 -->
				<!-- 页面内容：头部——结束 -->
				<!-- 页面内容：列表绑定——开始 -->
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/role/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenRole/getSevenRoleList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'add_url': '${pageContext.request.contextPath}/sevenRole/toAdd',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
				'edit_url': '${pageContext.request.contextPath}/sevenRole/toUpdate',
				'cancel': '${pageContext.request.contextPath}/sevenRole/deleteSevenRole',                  //数据删除方法接口
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
