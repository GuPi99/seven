...
<html>
	...
	<body>
		...
		<div id="app">
			<div class="zyc_content">
				...
				<ul class="zyc_clear zyc_handle" id="zyc_adds">
					<li><button id="zyc_add" class="layui-btn layui-btn-normal"><i class="iconfont iconxinzeng"></i>添加文件</button></li>
				</ul>
				<!-- 页面内容：头部——按钮组 结束 -->
				<!-- 页面内容：头部——结束 -->
				<!-- 页面内容：列表绑定——开始 -->
		...
		<script type="text/html" id="zyc_btn">
			{{#  if(d.pfType == 'pdf'){ }}
			<a class="layui-btn layui-btn-xs" title="预览" href="#" lay-event="preview">预览</a>
			{{#  } }}
			<a class="layui-btn layui-btn-xs" download="{{ d.pfName }}.{{d.pfType}}" title="下载" href="${pageContext.request.contextPath}/{{d.pfUrl}}">下载</a>
		</script>
		<!-- 列表：操作按钮绑定——结束 -->
		<!-- jquery文件 -->
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/file/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenProjectFile/getSevenProjectFileList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'add_url': '${pageContext.request.contextPath}/sevenProjectFile/toAdd',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
