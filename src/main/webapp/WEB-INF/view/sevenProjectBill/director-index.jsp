...
<html>
	...
	<body>
			...
			<div class="zyc_content">
				...
				<ul class="zyc_query zyc_clear">
					<form class="layui-form" id="zyc_search" style="display: inline-block;">
						...
						<input type="hidden" name="pbState" value="1" />
						<!-- 页面内容：头部——查询框或下拉框 结束 -->
					</form>
					<!-- 页面内容：头部——查询按钮 开始 -->
					<li>
		...
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenProjectBill/getSevenProjectBillList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'selects': '${pageContext.request.contextPath}/sevenFirm/selectAll',                //模糊搜索下拉框数据方法接口
				'select_url': '${pageContext.request.contextPath}/sevenProjectBill/directorSelect',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
