...
<html>
	...
	<body>
		...
		<div id="app">
			...
			<form class="layui-form" lay-filter="formTest">
				...
				<div class="layui-form-item">
					<!-- *表示必填，非必填项可将span标签删除 -->
					<label class="layui-form-label"><span>*</span>账户名称 : </label>
					<!-- 输入框或下拉框 -->
					<div class="layui-input-block">
						<input type="text" name="aAccount" placeholder="请输入账户名称" id="aAccount" onblur="checkName()" class="layui-input">
						<span id="remind" color="red"></span>
					</div>
					<!-- 验证信息展示处，data-name对应验证数据 -->
					<div class="zyc_verify" data-name="aAccount"></div>
				</div>
				<!-- 表单XX信息——结束 -->
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<input type="password" name="aPassword" placeholder="请输入密码" class="layui-input">
					</div>
					<div class="zyc_verify" data-name="aPassword"></div>
				</div>
				<div class="layui-form-item">
					...
					<div class="zyc_verify" data-name="aRid"></div>
				</div>
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>使用员工: </label>
					<div class="layui-input-block">
						<select name="aSid">
						</select>
					</div>
					<div class="zyc_verify" data-name="aSid"></div>
				</div>
				<!-- 提交按钮——开始 -->
				<div class="layui-form-item">
					<div class="layui-input-block">
						<input type="hidden" name="id" />
						<button lay-filter="formDemo" lay-submit class="layui-btn" id="commit">立即提交</button>
		...
		<script>
			var SCOPE = {
				'selects': '${pageContext.request.contextPath}/sevenRole/selectAllRole',//角色数据获取
				'operate': '${pageContext.request.contextPath}/sevenAccount/addSevenAccount',//数据提交接口
			}
						...
						success: function(result) {
							if (result.status == 0) {
								...
								$(datas).each(function(i){
									li.push('<option value="'+datas[i].id+'">'+datas[i].rName+'</option>');
								})
								var lis = li.join("");
								$('select[name=aRid]').html(lis);
								var datan = result.datan;
								var lin = [];
								$(datan).each(function(i){
									lin.push('<option value="'+datan[i].id+'">'+datan[i].sName+'</option>');
								})
								var lins = lin.join("");
								$('select[name=aSid]').html(lins);
								form.render('select');
							} else {
								var msg = result.msg;
								layer.msg(msg);
			...
			function checkName(){
				var aAccount=$("#aAccount").val();
				 $.ajax({
				        type: "POST",
				        url: "${pageContext.request.contextPath}/sevenAccount/checkName",
				        data:{"aAccount":aAccount},
				        dataType:"json",
				        success: function(msg){
				        	if (msg.status=="error") {
				        		document.getElementById("commit").disabled=true;
				                $("#remind").html("<font color='red'>用户名已存在，请更换！</font>");
