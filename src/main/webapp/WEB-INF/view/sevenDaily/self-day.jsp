...
<!-- 页面内容——开始 -->
		...
		<script>
			layui.use(['laydate','form','table','layer','element'], function(){
				...
				var url='${pageContext.request.contextPath}/sevenDaily/getSelfDayList';
				var data=new Object();
			    data.dDate='';
				search(url,data);
				...
				form.on('submit(add)',function(data){
					$("#open",parent.document).attr('src','${pageContext.request.contextPath}/sevenDaily/toDayAdd');
					return false;
				})
				table.on('tool(test)', function(obj){
					var data = obj.data;
					if(obj.event === 'detail'){
						$("#open",parent.document).attr('src','${pageContext.request.contextPath}/sevenDaily/toDaySelfSelect?id='+data.id);
					}
				});
