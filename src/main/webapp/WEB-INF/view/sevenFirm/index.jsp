...
<html>
	...
	<body>
		...
		<div id="app">
			<div class="zyc_content">
					...
					<form class="layui-form" id="zyc_search" style="display: inline-block;">
						...
						<li>
							<div class="layui-form-item">
								<input type="text" name="fName" placeholder="请输入客户名称" autocomplete="off" class="layui-input">
							</div>
						</li>
						<!-- 页面内容：头部——查询框或下拉框 结束 -->
					</form>
				...
				<ul class="zyc_clear zyc_handle">
					<li><button id="zyc_add" class="layui-btn layui-btn-normal"><i class="iconfont iconxinzeng"></i>添加客户</button></li>
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/sevenFirm/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenFirm/getSevenFirmList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'add_url': '${pageContext.request.contextPath}/sevenFirm/toAdd',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
				'edit_url': '${pageContext.request.contextPath}/sevenFirm/toUpdate',	
				'cancel': '${pageContext.request.contextPath}/sevenFirm/deleteSevenFirm',                  //数据删除方法接口
				'into': '${pageContext.request.contextPath}/sevenFirm/toInto',                     //批量导入
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
