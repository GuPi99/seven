...
<html>
	...
	<body>
		...
		<div id="app">
			...
			<form class="layui-form" lay-filter="formTest">
				...
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>产品类型 : </label>
					<div class="layui-input-block">
						<select name="pgCid">
							<option value=""></option>
						</select>
					</div>
					<div class="zyc_verify" data-name="pg_pid"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>系统产品 : </label>
					<div class="layui-input-block">
						<select name="pgGid">
							<option value=""></option>
						</select>
					</div>
					<div class="zyc_verify" data-name="pg_gid"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>自定义名称 : </label>
					<div class="layui-input-block">
						<input type="text" name="psName" placeholder="请输入自定义名称" class="layui-input">
					</div>
					<div class="zyc_verify" data-name="ps_name"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>自定义型号 : </label>
					<div class="layui-input-block">
						<input type="text" name="pgModel" placeholder="请输入自定义型号" class="layui-input">
					</div>
					<div class="zyc_verify" data-name="pg_model"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>需求数量 : </label>
					<div class="layui-input-block">
						<input type="number" name="pgNum" placeholder="请输入需求数量" class="layui-input">
					</div>
					<div class="zyc_verify" data-name="pg_num"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label">备注 : </label>
					<div class="layui-input-block">
						<textarea name="pgNote" placeholder="请输入备注" class="layui-textarea"></textarea>
					</div>
					<div class="zyc_verify" data-name="pg_note"></div>
				</div>
		...
		<script>
			var SCOPE = {
				'selects': '${pageContext.request.contextPath}/sevenGoods/selectAllCG',//类型下拉框数据获取
				'operate': '${pageContext.request.contextPath}/sevenProjectGoods/addSevenProjectGoods',//数据提交接口
				...
				$(function() {
					...
					$.ajax({
						...
						success: function(result) {
							if (result.status == 0) {
								var datas = result.type;
								var li = [];
								$(datas).each(function(i){
									li.push('<option value="'+datas[i].id+'">'+datas[i].cName+'</option>');
								})
								var lis = li.join("");
								$('select[name=pgCid]').html(lis);
								form.render('select');
								goods($('select[name=pgPid]').val());
								form.on('select(pgPid)', function(data){
									goods(data.value);
								});      
							} else {
								var msg = result.msg;
					...
					$.ajax({
						...
						success: function(result) {
							if (result.status == 0) {
								...
								var li = [];
								$(datas).each(function(i){
									li.push('<option value="'+datas[i].id+'">'+datas[i].gName+'</option>');
								})
								var lis = li.join("");
								$('select[name=pgGid]').html(lis);
								form.render('select');
							} else {
								var msg = result.msg;
								layer.msg(msg);
