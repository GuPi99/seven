...
<html>
	...
	<body>
		...
		<script type="text/html" id="zyc_btn">
			<a class="layui-btn layui-btn-xs" title="查看配件信息" href="#" lay-event="view">查看配件信息</a>
			<a class="layui-btn layui-btn-xs layui-btn-danger" title="删除" href="#" lay-event="delete">删除</a>
		</script>
		<script type="text/html" id="zyc_btns">
			<a class="layui-btn layui-btn-xs" title="查看配件信息" href="#" lay-event="view">查看配件信息</a>
		</script>
		<!-- 列表：操作按钮绑定——结束 -->
		<!-- jquery文件 -->
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/demand/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenProjectGoods/getSevenProjectGoodsList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'add_url': '${pageContext.request.contextPath}/sevenProjectGoods/toAdd',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
				'cancel': '${pageContext.request.contextPath}/sevenProjectGoods/delectProjectGoods',                  //数据删除方法接口
				'educe': '${pageContext.request.contextPath}/sevenProjectGoods/educe',
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
