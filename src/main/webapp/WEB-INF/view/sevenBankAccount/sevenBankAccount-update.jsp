...
<html>
	...
	<body>
		...
		<div id="app">
			...
			<form class="layui-form" lay-filter="formTest">
				...
				<div class="layui-form-item">
					...
					<div class="zyc_verify" data-name="ba_fid"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>银行 : </label>
					<div class="layui-input-block">
						<select name="baBid">
						</select>
					</div>
					<div class="zyc_verify" data-name="ba_bid"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>账户 : </label>
					<div class="layui-input-block">
						<input type="text" name="baAccount" placeholder="请输入账户号码" class="layui-input">
					</div>
		...
		<script>
			var SCOPE = {
				'selects': '${pageContext.request.contextPath}/sevenBankAccount/selectAllBankAndFirm', //下拉框数据方法接口
				'operate': '${pageContext.request.contextPath}/sevenBankAccount/updateSevenBankAccount',//数据提交接口
				'value': '${pageContext.request.contextPath}/sevenBankAccount/selectSevenBankAccountById',//编辑时，获取单一数据接口，用以展示编辑数据
			}
					...
					$.ajax({
						...
						success: function(result) {
							if (result.status == 0) {
								...
								$("select[name=baFid]").val(datas.baFid);//下拉框赋值
								$("select[name=baBid]").val(datas.baBid);//下拉框赋值
								form.render(); //重新渲染
							} else {
								var msg = result.msg;
								layer.msg(msg);
