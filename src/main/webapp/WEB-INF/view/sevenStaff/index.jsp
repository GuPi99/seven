...
<html>
	...
	<body>
		...
		<div id="app">
			<div class="zyc_content">
					...
					<form class="layui-form" id="zyc_search" style="display: inline-block;">
						...
						<li>
							<div class="layui-form-item">
								<select name="sSex" lay-filter="caidan" lay-search>
						...
						<li>
							<div class="layui-form-item">
								<input type="text" name="sDateBirth" placeholder="请选择员工生日日期" autocomplete="off" class="layui-input date">
							</div>
						</li>
						<!-- 页面内容：头部——查询框或下拉框 结束 -->
					</form>
				...
				<ul class="zyc_clear zyc_handle">
					<li><button id="zyc_add" class="layui-btn layui-btn-normal"><i class="iconfont iconxinzeng"></i>添加员工</button></li>
				</ul>
				<!-- 页面内容：头部——按钮组 结束 -->
				<!-- 页面内容：头部——结束 -->
				<!-- 页面内容：列表绑定——开始 -->
		...
		<script type="text/html" id="zyc_btn">
			<a class="layui-btn layui-btn-xs" title="查看" href="#" lay-event="check">查看</a>
			{{#  if(d.sState == '1'){ }}
			<a class="layui-btn layui-btn-xs layui-btn-warm" title="离职" href="#" lay-event="state">离职</a>
			{{#  } }}
			{{#  if(d.sState == '0'){ }}
			<a class="layui-btn layui-btn-xs layui-btn-warm" title="复职" href="#" lay-event="state">复职</a>
			{{#  } }}
		</script>
		<!-- 列表：操作按钮绑定——结束 -->
		<!-- jquery文件 -->
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/staff/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenStaff/getSevenStaffList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'edit_url': '${pageContext.request.contextPath}/sevenStaff/toUpdate',
				'add_url': '${pageContext.request.contextPath}/sevenStaff/toAdd',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
				'operate': '${pageContext.request.contextPath}/sevenStaff/updateSevenStaffState',
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
