...
<html>
	...
	<body>
		...
		<script type="text/html" id="zyc_btns">
			<a class="layui-btn layui-btn-xs" title="回收" href="#" lay-event="recycle">回收</a>
		</script>
		<!-- 列表：操作按钮绑定——结束 -->
		<!-- jquery文件 -->
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/fund/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenProjectFundPlan/getSevenProjectFundPlanList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'add_url': '${pageContext.request.contextPath}/sevenProjectFundPlan/toAdd',					  //新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
				'edit_url': '${pageContext.request.contextPath}/sevenProjectFundPlan/toUpdate',
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
