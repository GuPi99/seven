...
<html>
	...
	<body>
		...
		<div id="app">
			<div class="zyc_content">
					...
					<form class="layui-form" id="zyc_search" style="display: inline-block;">
						...
						<li>
							<div class="layui-form-item">
								<select name="gCid" lay-filter="caidan" lay-search>
						...
						<li>
							<div class="layui-form-item">
								<input type="text" name="gName" placeholder="请输入产品名称" autocomplete="off" class="layui-input">
							</div>
						</li>
						<!-- 页面内容：头部——查询框或下拉框 结束 -->
					</form>
				...
				<ul class="zyc_clear zyc_handle">
					<li><button id="zyc_add" class="layui-btn layui-btn-normal"><i class="iconfont iconxinzeng"></i>添加产品</button></li>
					<li><button id="zyc_into" class="layui-btn layui-btn-normal"><i class="iconfont icongengxin"></i>批量添加</button></li>
				</ul>
				<!-- 页面内容：头部——按钮组 结束 -->
				<!-- 页面内容：头部——结束 -->
				<!-- 页面内容：列表绑定——开始 -->
		...
		<!-- 页面操作js文件 -->
		<script src="${pageContext.request.contextPath}/static/js/goods/load.js"></script>
		<script>
			var SCOPE = {
				'table_url': '${pageContext.request.contextPath}/sevenGoods/getSevenGoodsList', //列表数据查询来源（json文件或接口地址，可自行修改）；模糊搜索查询接口地址（menu指当前控制器，selects指查询方法，可自行修改）
				'selects': '${pageContext.request.contextPath}/sevenClass/selectAll',                 //模糊搜索下拉框数据方法接口
				'add_url': '${pageContext.request.contextPath}/sevenGoods/toAdd',	//新增页面跳转路径（menu指当前控制器，add指页面或控制器方法，可自行修改）
				'edit_url': '${pageContext.request.contextPath}/sevenGoods/toUpdate',	
				'cancel': '${pageContext.request.contextPath}/sevenGoods/deleteSevenGoods',                  //数据删除方法接口
				'into': '${pageContext.request.contextPath}/sevenGoods/toInto',                     //批量导入
			}
			//关闭所有弹框，并重新加载当前页面
			function cloae(){
				layer.closeAll();
