...
<html>
	...
	<body>
		...
		<div id="app">
			...
			<form class="layui-form" lay-filter="formTest">
				...
				<div class="layui-form-item">
					...
					<div class="zyc_verify" data-name="g_cid"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>名称 : </label>
					<div class="layui-input-block">
						<input type="text" name="gName" placeholder="请输入产品名称" class="layui-input">
					</div>
					<div class="zyc_verify" data-name="g_name"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>型号 : </label>
					<div class="layui-input-block">
						<input type="text" name="gModel" placeholder="请输入产品型号" class="layui-input">
					</div>
					<div class="zyc_verify" data-name="g_model"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>归属产品 : </label>
					<div class="layui-input-block">
						<select name="gGid">
							<option value=""></option>
						</select>
					</div>
					<div class="zyc_verify" data-name="g_gid"></div>
				</div>
				
				<div class="layui-form-item">
					<label class="layui-form-label"><span>*</span>描述 : </label>
					<div class="layui-input-block">
						<textarea name="gDescribe" placeholder="请输入描述" class="layui-textarea"></textarea>
					</div>
					<div class="zyc_verify" data-name="g_describe"></div>
				</div>
		...
		<script>
			var SCOPE = {
				'selects': '${pageContext.request.contextPath}/sevenGoods/selectAllCG',//下拉框数据获取
				'operate': '${pageContext.request.contextPath}/sevenGoods/updateSevenGoods',//数据提交接口
				'value': '${pageContext.request.contextPath}/sevenGoods/selectSevenGoodsById',//编辑时，获取单一数据接口，用以展示编辑数据
			}
					...
					$.ajax({
						...
						success: function(result) {
							if (result.status == 0) {
								...
								var lis = li.join("");
								$('select[name=gGid]').html(lis);
								var types = result.type;
								var lin = [];
								$(types).each(function(i){
									lin.push('<option value="'+types[i].id+'">'+types[i].cName+'</option>');
								})
								var lins = lin.join("");
								$('select[name=gCid]').html(lins);
								form.render('select');
							} else {
								var msg = result.msg;
								layer.msg(msg);
					...
					$.ajax({
						...
						success: function(result) {
							if (result.status == 0) {
								...
								$("select[name=gGid]").val(datas.gGid);//下拉框赋值
								$("select[name=gCid]").val(datas.gCid);//下拉框赋值
								form.render(); //下拉框重新渲染
							} else {
								var msg = result.msg;
								layer.msg(msg);
