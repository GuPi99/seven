	...
	<body>
		...
		<script>
			var SCOPE = {
				...
				'operate': '${pageContext.request.contextPath}/sevenContact/updateSevenContact',//数据提交接口
				'value'  : '${pageContext.request.contextPath}/sevenContact/selectSevenContactById'
			}
					...
					$.ajax({
						...
						success: function(result) {
							if (result.status == 0) {
								...
								$("select[name=cFid]").val(datas.cFid);//下拉框赋值
								form.render(); //下拉框重新渲染
							} else {
								var msg = result.msg;
								layer.msg(msg);
