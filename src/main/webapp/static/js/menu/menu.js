			...
			if (result.status == 0) {
				...
				$(datas).each(function(i){
					if(datas[i].mMid == 0 && datas[i].mName == '首页'){
						var lis = '<li ';
						lis += 'class="zyc_this"';
						lis += '><a class="zyc_me_menu" href="'+datas[i].mUrl+'"><i class="iconfont '+datas[i].mTag+'"></i><span>'+datas[i].mName+'</span></a></li>';
						li.push(lis);
					}else if(datas[i].mMid == 0 && datas[i].mName != '首页'){
						var lis = '<li class="zyc_twin"><a class="zyc_me_menu"><i class="iconfont '+datas[i].mTag+'"></i><span>'+datas[i].mName+'</span></a>';
						lis += '<ul class="zyc_clear zyc_menu_twin">';
						var liss = [];
						$(datas).each(function(k){
							if(datas[i].id == datas[k].mMid){
								liss.push('<li><a class="zyc_me_menu" href="'+datas[k].mUrl+'"><i class="iconfont '+datas[k].mTag+'"></i><span>'+datas[k].mName+'</span></a></li>');
							}else{
								liss.push('');
							}
						})
